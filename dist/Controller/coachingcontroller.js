...
export const getCoachingTimings = async (req, res) => {
    try {
...
export const createCoachingTiming = async (req, res) => {
    try {
        ...
        let timingSport = 'Party'; // Default sport
        if (sport) {
            const validSports = ['Cricket', 'Football', 'Pickleball', 'Gaming', 'Badminton', 'Party'];
...
export const updateCoachingTiming = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
...
export const deleteCoachingTiming = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ error: 'Invalid timing ID' });
